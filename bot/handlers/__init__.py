...
def register_all(app: Application) -> None:  # type: ignore[type-arg]
    ...
    # Apply rate limiting to command handlers
    app.add_handler(CommandHandler("start", rate_limit_middleware(start_command)))
    app.add_handler(CommandHandler("tasks", rate_limit_middleware(show_tasks)))
